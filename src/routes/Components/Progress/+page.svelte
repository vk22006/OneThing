...
<div class="px-8 py-8 max-w-5xl mx-auto w-full">
    ...
    <div class="grid grid-cols-4 gap-4 mb-10">
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-[14px] shadow-sm p-5 hover:shadow-md transition-shadow">
            <p class="text-[13px] font-bold text-[var(--muted-2)] uppercase tracking-wide mb-1.5">Projects</p>
            <p class="text-3xl font-bold text-[var(--text)]">{totalProjects}</p>
        </div>
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-[14px] shadow-sm p-5 hover:shadow-md transition-shadow">
            <p class="text-[13px] font-bold text-[var(--muted-2)] uppercase tracking-wide mb-1.5">Tasks</p>
    ...
    <h2 class="text-lg font-bold text-[var(--text)] tracking-tight mb-5">Project Breakdown</h2>
    ...
    <div class="grid grid-cols-2 gap-4">
        {#each projects as project}
            ...
            <div class="bg-[var(--surface)] border border-[var(--border)] rounded-[14px] shadow-sm p-5 hover:shadow-md transition-all cursor-default">
                <div class="flex justify-between mb-3.5">
                    <h4 class="font-semibold text-[var(--text)] translate-y-[-1px]">{project.title}</h4>
        ...
        {#if projects.length === 0}
            <div class="col-span-2 text-center py-12 border-2 border-dashed border-[var(--border)] rounded-2xl bg-[var(--surface-3)]">
