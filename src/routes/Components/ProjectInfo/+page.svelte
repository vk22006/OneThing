...
<div class="px-8 py-8 max-w-5xl mx-auto w-full">
    <!-- Add Project Form -->
    <div class="bg-white border border-gray-200 rounded-[14px] shadow-sm p-5 mb-8 flex gap-3">
        <div class="flex-1 relative">
            <input
                bind:value={newProject}
                placeholder="Project Title"
                class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm placeholder-gray-400 focus:outline-none focus:ring-[3px] focus:ring-blue-100 focus:border-blue-500 focus:bg-white transition-all shadow-sm"
            />
        </div>
    ...
    <div class="flex flex-col gap-6">
        {#each projects as project (project.id)}
            <div 
                ...
                class="bg-white border border-gray-200 rounded-[14px] shadow-sm overflow-hidden"
            >
                <!-- Project Header -->
                <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 tracking-tight mb-1.5">{project.title}</h2>
                        <div class="flex items-center gap-1.5 text-[13px] text-gray-500 font-medium">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            Deadline: <span class="text-gray-700">{project.deadline || 'No deadline'}</span>
                        </div>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-5">
                        <div class="flex justify-between items-end mb-2.5">
                            <span class="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Progress</span>
                    ...
                    <div class="flex flex-col gap-2 mb-6">
                        <h3 class="text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-1">Tasks</h3>
                        {#each project.tasks as task (task.id)}
                            <div class="group flex items-center gap-3 p-3 bg-gray-50/50 border border-gray-100 rounded-[10px] hover:border-gray-200 transition-colors">
                                <button 
                                    onclick={() => toggleTask(project.id, task.id)}
                                    class="flex-shrink-0 w-5 h-5 rounded-[6px] border-[1.5px] flex items-center justify-center transition-colors {task.status === 'DONE' ? 'bg-blue-600 border-blue-600' : 'border-gray-300 hover:border-blue-500'}"
                                ...
                                <span class="flex-1 text-[13.5px] {task.status === 'DONE' ? 'text-gray-400 line-through' : 'text-gray-700 font-medium'} select-none">
                                    {task.task}
                                </span>
                                <button 
                                    onclick={() => deleteTask(project.id, task.id)}
                                    class="flex-shrink-0 p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-[6px] opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all"
                        ...
                        <div class="relative mt-1.5">
                            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                <svg class="w-[18px] h-[18px] text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                            </div>
                            <input
                                placeholder="Add a new task..."
                                class="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-[10px] text-[14px] placeholder-gray-400 focus:outline-none focus:ring-[3px] focus:ring-blue-100 focus:border-blue-500 shadow-sm transition-all"
                                onkeydown={(e)=>{
                                    if(e.key==="Enter") {
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <h3 class="text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                        ...
                        <textarea
                            ...
                            class="w-full bg-gray-50/50 border border-gray-100 rounded-[10px] px-3.5 py-3 text-[13.5px] text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-[3px] focus:ring-blue-100 focus:border-blue-200 focus:bg-white transition-all resize-none min-h-[100px]"
                        ></textarea>
                    </div>
                </div>
            </div>
        ...
        {#if projects.length === 0}
            <div class="text-center py-16 border-2 border-dashed border-gray-200 rounded-[14px] bg-gray-50/50">
                ...
                <h3 class="text-[15px] font-semibold text-gray-900">No Projects</h3>
                <p class="text-[13px] text-gray-500 mt-1">Create a new project above to organize your tasks.</p>
            </div>
        {/if}
    </div>
</div>
