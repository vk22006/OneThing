...
function getInitialTheme(): Theme {
    ...
    try {
        ...
        if (stored?.toLowerCase() === "light") return "light";
        if (stored?.toLowerCase() === "dark") return "dark";
    } catch {
...
function applyThemeToDOM(value: string) {
    let debugText = "applyThemeToDOM called with: " + value + " | has_doc: " + (typeof document !== 'undefined');
    ...
    if (isTauri) {
        try {
            ...
            theme.subscribe(value => {
                ...
                store.set("theme", value).then(() => store.save()).catch(console.error);
            });
            return;
        } catch (e) {
